...
### 通信開始の修正（HomeHub 0.3.3 / SelfCare 0.3.10）

HEM-6232Tのキー登録後に同じキーで認証してから通信開始し、開始・終了は8バイトの制御応答として検証します。RX通知を先に購読して待ち時間を設け、TXはGATTの書込み属性に合わせます。接続中かつ受信断片がない通信開始の時間切れだけを1回再試行します。診断には `protocol_revision: 2`、送信方式、RX通知数、時間切れ時の接続状態を返します。キー登録や履歴読取の再試行、測定EEPROMの書込みは追加しません。実機での結果は更新後にSelfCareの診断付きペアリングで確認してください。
