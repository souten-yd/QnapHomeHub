...
async function loadJson(file, fallback) {
  try { return { ...fallback, ...JSON.parse(await fs.readFile(file, 'utf8')) }; }
...
async function checkForUpdates({ triggerAuto = true } = {}) {
  ...
  state.phase = 'checking'; state.lastError = null;
  ...
  try {
    ...
    await saveJson(stateFile, state);
    log('Release check complete', { current, latest, updateAvailable: state.updateAvailable, autoUpdate: config.autoUpdate });
    if (triggerAuto && config.autoUpdate && state.updateAvailable) setTimeout(() => void applyUpdate(release.tag_name, true), 250).unref();
    return statusPayload();
  } catch (error) {
    state.phase = 'error'; state.lastCheckedAt = new Date().toISOString(); state.lastError = error.message;
    await saveJson(stateFile, state);
    log('Release check failed', error.message);
    throw error;
  }
}
...
async function composeUp() {
  await command([
    ...
    'up', '-d', '--force-recreate', 'homehub', 'matterbridge',
  ]);
}

async function waitForHealthy(targetVersion, timeoutMs = 120000) {
  const deadline = Date.now() + timeoutMs;
  let last = 'waiting';
  while (Date.now() < deadline) {
    try {
      const [home, matter] = await Promise.all([
        fetch('http://127.0.0.1:8787/api/health', { signal: AbortSignal.timeout(2500) }),
        fetch('http://127.0.0.1:8283/health', { signal: AbortSignal.timeout(2500) }),
      ]);
...
async function applyUpdate(tag = state.latestTag, automatic = false) {
  if (busy) return;
  ...
  try {
    ...
    log(`Starting ${automatic ? 'automatic' : 'manual'} update to ${tag}`, { oldServer, oldMatter });
    ...
    await saveJson(stateFile, state);
    log(`Update to ${tag} completed`);
  } catch (error) {
    log(`Update to ${tag} failed; attempting rollback`, error.message);
    ...
    try {
      ...
      if (oldServer && oldMatter) {
        await composeUp();
        await waitForHealthy(null, 90000);
      }
      state.phase = 'error';
...
function statusPayload() {
  return { ...state, busy, autoUpdate: Boolean(config.autoUpdate), checkIntervalMs, repository };
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/apply') {
      ...
      setTimeout(() => void applyUpdate(tag, false), 50).unref();
      return send(res, 202, { accepted: true, tag });
    }
...
setTimeout(() => void checkForUpdates().catch(() => undefined), 15000).unref();
setInterval(() => void checkForUpdates().catch(() => undefined), checkIntervalMs).unref();
