...
async function refreshRadioStatus() {
  ...
  if (!node) return;
  try { const radio = await api('/api/radio'); node.textContent = radio.shared ? `共通Bluetooth · SelfCare中心 · 実行: ${radio.active} · 待機: ${radio.pending}${radio.bluezError ? ` · SelfCare復帰エラー: ${radio.bluezError}` : ""}` : '従来の直接Bluetooth接続'; }
