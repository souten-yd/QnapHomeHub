...
0.3.4では実機で確認された24バイトの通信開始応答（機器情報付き）に対応します。0.3.3ではHEM-6232Tのキー登録後の認証・通信開始応答の検査を修正し、通知待ち時間と送信方式を調整します。0.3.2では診断付き操作の直近8件の機器応答とタイムアウト時の受信断片を返します。0.3.1ではSelfCareの手動診断付きペアリング・同期に対応し、測定データの限定サンプル・読取段階をradioからSelfCareへ返します。平常の同期結果・Bot操作は変わりません。診断値はSelfCareの履歴に保存されるため共有前に確認してください。
